...
class MultiLayerPerception(object):
    ...
    def __init__(self, L=2): 
        ...
        for i in range(1,L): # 当L=1时该循环不执行
            z.append(1/(1+T.exp(-n[i-1]))) # sigmoid函数
            n.append(T.dot(z[i-1],w[i])+b[i])
            
        model_predict = n[-1] # 模型的预测
        self.f_model_predict = theano.function([x]+w+b,model_predict)
        
        object_function = 0.5 * ((model_predict - y)**2).sum() + 1e-5 * sum([(w_i**2).sum() for w_i in w])
...
if __name__ == '__main__':
    ...
    x_optimal,y_optimal = optimal.minimize_GD(model,w1,w2,w3,b1,b2,b3,max_step=50000,learn_rate=1e-4)
